...
gltbMouse(int button, int state, int x, int y)
{
  ...
  if (state == GLUT_DOWN && button == gltb_button)
    _gltbStartMotion(x, y, button, glutGet(GLUT_ELAPSED_TIME));//glutGet(GLUT_ELAPSED_TIME));
  else if (state == GLUT_UP && button == gltb_button)
    _gltbStopMotion(button, glutGet(GLUT_ELAPSED_TIME));//glutGet(GLUT_ELAPSED_TIME));
}
...
gltbMotion(int x, int y)
{
  ...
  /* reset for next time */
  gltb_lasttime = glutGet(GLUT_ELAPSED_TIME);//glutGet(GLUT_ELAPSED_TIME);
